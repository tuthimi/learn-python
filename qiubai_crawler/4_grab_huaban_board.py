...
def BoardGetPins(board):
    ...
    data = requests.get(url, timeout=10, verify=False, headers=headers).text.encode('utf-8')
    pins = [ _[-1] for _ in re.findall(pin_pat, data) if _[-1] ] # list[-1] list中最后一个（倒数第一个元素）
    while 1:
        #get ajax pin data
...
def GetUserBoards(user, limit=50):
    ...
    try:
        ...
        if r.status_code == 200:
            try:
                print r.text
                print r.text.split('app.page["user"] = ')[-1].split("app.route();")[0].split("app._csr")[0].strip(";\n")
...
def main(users=None, boards=None, processes=6):
    ...
    if users:
        ...
        worker = []
        for user in users:
            p = Process(target=ExecuteDownloadUser, args=(user, int(processes)), name="grab.user.{}.huaban".format(user)) # 'grab.user.123123123123.huaban'
            #p.daemon=True
            worker.append(p)
        for p in worker: p.start()
        for p in worker: p.join()
